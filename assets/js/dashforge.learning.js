$(function(){
  ...
  $('#contactContentHide2').on('click touch', function(e){
    e.preventDefault();

    $('body').toggleClass('contact-content-show');
    // $('body').toggleClass('contact-options-show', false);
    $('body').toggleClass('contact-content-visible');
    $('#refresh-width').toggleClass('full-width');
    // $("#refresh-width").toggle(
    //   function(){$("refresh-width").css({"width": $(window).width() });},
    //   function(){$("refresh-width").css({"width": '' });}
    // );
    // $("#refresh-width").width('100%');
    // $("#refresh-width").load(location.href+" #refresh-width>*","");
    // $('#refresh-width').removeClass("contact-content").addClass("contact-content");
    // $('#mainMenuOpen').removeClass('d-none');
    // $(this).addClass('d-none');
